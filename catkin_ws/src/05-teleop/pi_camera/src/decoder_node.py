...
class DecoderNode(object):
    ...
    def cbImg_old(self,msg):
        print "[DECODER] XXXXXXXXXXXX    1     XXXXXXXXXXXX"
        if not self.active:
            print "[DECODER] XXXXXXXXXXXX    not self.active     XXXXXXXXXXXX"
            return
        now = rospy.Time.now()
        if now - self.last_stamp < self.publish_duration:
            print "[DECODER] XXXXXXXXXXXX    less than min allowed time     XXXXXXXXXXXX"
            return
        else:
            self.last_stamp = now
    ...
    def cbImg(self,msg):
        ...
        # rospy.loginfo("[%s] Took %f sec to conver to Image."%(self.node_name,time_2 - time_1))
        # rospy.loginfo("[%s] Took %f sec to publish."%(self.node_name,time_3 - time_2))
        print "[DECODER] XXXXXXXXXXXX    2     XXXXXXXXXXXX"
